...
from atorch.distributed.distributed import get_data_partition_rank_and_size, parallel_group
...
def parse_args():
    ...
    parser.add_argument("--clip_pseudo_grad", type=int, default=None, required=False)
    parser.add_argument("--gradnorm_weighted", default=False, action="store_true")
    parser.add_argument("--skip_anomaly", default=False, action="store_true")
    parser.add_argument("--skip_anomaly_warmup_steps", type=int, default=10, required=False)
...
def train(args):
    ...
    if args.use_local_sgd:
        local_sgd_config = LocalSGDConfig(
            ...
            clip_pseudo_grad=args.clip_pseudo_grad,
            pseudo_gradnorm_reduce=args.gradnorm_weighted,
            skip_anomaly=args.skip_anomaly,
            ewma_warmup_steps=args.skip_anomaly_warmup_steps,
        )
        ...
        gta_config = GTAConfig(reducer="gta")
    if args.load_strategy:
        # data parallel if distributed
        strategy = ["parallel_mode"] if args.distributed else []
        if args.use_local_sgd and args.use_fsdp:
            # With FSDP + local sgd, we need to use HSDP mode
        ...
        # module_replace # NOTE lots of bugs, be very cautious
        ...
        if args.use_fsdp:
            ...
            if args.use_local_sgd:
                ...
                fsdp_config["local_sgd_config"] = local_sgd_config
                fsdp_config["outer_optim_config"] = outer_optim_config
                fsdp_config["gta_config"] = gta_config
    ...
    if args.use_local_sgd and not args.use_fsdp:
