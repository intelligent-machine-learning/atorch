...
def generate_gradnorm_curve(
    ...
    gradnorms = torch.zeros(steps)
    labels = torch.zeros(steps)  # 0 for normal, 1 for anomaly
...
class AnomalyDetectionTest(unittest.TestCase):
    def test_run_anomaly_detection(self):
        from atorch.local_sgd.utils import OnlineDynamicEWMA

        anomaly_detector = OnlineDynamicEWMA(alpha=0.03, warmup_steps=100, base_threshold=3)
        gradnorm_curve, gradnorm_labels = generate_gradnorm_curve()
        outliers = []
