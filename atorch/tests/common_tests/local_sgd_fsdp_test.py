...
def compare_fsdp_modules(fsdp_modules1, fsdp_modules2):
    ...
    for i in range(len(fsdp_modules2)):
        if fsdp_modules1[i].global_step != fsdp_modules2[i].global_step:
            return False
        if fsdp_modules1[i]._handle.use_outer_optim != fsdp_modules2[i]._handle.use_outer_optim:
            return False
        if fsdp_modules1[i]._handle.use_outer_optim and fsdp_modules1[i]._handle.outer_optimizer is not None:
            if not compare_dicts(
                fsdp_modules1[i]._handle.outer_optimizer.state_dict(),
                fsdp_modules2[i]._handle.outer_optimizer.state_dict(),
            ):
                return False
            if not torch.equal(
                fsdp_modules1[i]._handle.last_synced_params, fsdp_modules2[i]._handle.last_synced_params
            ):
                return False
    return True
...
class TestLocalSGD(unittest.TestCase):
    def setUp(self):
    ...
    @unittest.skipIf(
        ...
        or not (torch_version()[0] == 2 and (torch_version()[1] == 1 or torch_version()[1] >= 4)),  # type: ignore
        "Must have at least 4 GPUs and torch == 2.1.x. or torch >= 2.4.x. for local sgd test.",
    )
